...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerEx(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, ImGuiIO& io)
{
    ...
    case WM_NCMOUSEMOVE:
    {
        ...
        {
            if (::ScreenToClient(hwnd, &mouse_pos) == FALSE)
                return 0; // 失敗したら無視
        }
        ...
        // ゲーム内部解像度にスケーリング
        float scaledX = (mouse_pos.x / clientWidth) * SCREEN_WIDTH;
        float scaledY = (mouse_pos.y / clientHeight) * SCREEN_HEIGHT;

        // ImGui にスケーリング済み座標を渡す
